...
//
// Параметры:
//   НовыйИндексПодписчиков - Массив - Подписчики в порядке приоритета
//                          - Соответствие - Новый индекс для выполнения подписок (для совместимости)
//                                            * ключ - имя подписчика
...
Процедура УстановитьПодписчиков(НовыйИндексПодписчиков) Экспорт
	...
	ЗаполнитьПодписчиковПоИндексу(НовыйИндексПодписчиков);
...
Процедура ЗаполнитьПодписчиковПоИндексу(НовыйИндексПодписчиков)

	Если НовыйИндексПодписчиков = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Если ТипЗнч(НовыйИндексПодписчиков) = Тип("Массив") Тогда
		Подписчики = НовыйИндексПодписчиков;
	Иначе
		Подписчики = Новый Массив;
		Для Каждого КлючИЗначение Из НовыйИндексПодписчиков Цикл
			Подписчики.Добавить(КлючИЗначение.Значение);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПриСозданииОбъекта(Знач ВходящийИндексПодписчиков = Неопределено)
	...
	ЗаполнитьПодписчиковПоИндексу(ВходящийИндексПодписчиков);
