...
Перем ПодписчикиПоПриоритету; // Массив
...
Процедура УстановитьПодписчиков(НовыйИндексПодписчиков) Экспорт
	...
	ЗаполнитьПодписчиковПоПриоритету();
...
Процедура ВыполнитьПодпискуНаСобытие(Знач ИмяПроцедурыВыполнения, ПараметрыПроцедуры)
	...
	Для Каждого ИмяКласса Из ПодписчикиПоПриоритету Цикл

		Подписчик = ИндексПодписчиков[ИмяКласса];
...
Процедура ЗаполнитьПодписчиковПоПриоритету()

	// Сортировка плагинов в порядке приоритета и текущего порядка в индексе
	ИндексПодписчиковСПриоритетом = Новый ТаблицаЗначений;
	ИндексПодписчиковСПриоритетом.Колонки.Добавить("ИмяКласса");
	ИндексПодписчиковСПриоритетом.Колонки.Добавить("Приоритет");
	ИндексПодписчиковСПриоритетом.Колонки.Добавить("Порядок");

	Для Каждого ЭлементИндекса Из ИндексПодписчиков Цикл

		Подписчик = ЭлементИндекса.Значение;
		СтрокаПриоритета = ИндексПодписчиковСПриоритетом.Добавить();
		СтрокаПриоритета.ИмяКласса = ЭлементИндекса.Ключ;
		СтрокаПриоритета.Приоритет = Подписчик.Приоритет();
		СтрокаПриоритета.Порядок = ИндексПодписчиковСПриоритетом.Количество();

	КонецЦикла;

	ИндексПодписчиковСПриоритетом.Сортировать("Приоритет,Порядок");
	ПодписчикиПоПриоритету = ИндексПодписчиковСПриоритетом.ВыгрузитьКолонку("ИмяКласса");
...
Процедура ПриСозданииОбъекта(Знач ВходящийИндексПодписчиков = Неопределено)
	...
	Если ВходящийИндексПодписчиков = Неопределено Тогда
		...
		ПодписчикиПоПриоритету = Новый Массив;
	Иначе
